...
router.get("/", async (req, res) => {
  try {
    ...
    await db.tx(async (t) => {
      ...
      const sql =
        "SELECT saves FROM projects_info pi WHERE pi.project_id = ${projectId}";
...
router.post("/", async (req, res) => {
  try {
    ...
    await db.tx(async (t) => {
      t.none(req.app.locals.schema_query);
      const sql = "SELECT * FROM add_save(${login}, ${projectId}, NULL, NULL)";
...
router.delete("/", async (req, res) => {
  try {
    ...
    await db.tx(async (t) => {
      t.none(req.app.locals.schema_query);
      const sql =
        "SELECT * FROM delete_save(${login}, ${projectId}, NULL, NULL)";
      const sqlParams = { login, projectId: Number(projectId) };
      console.log(sqlParams);
      resDB = await t.one(sql, sqlParams);
    });
